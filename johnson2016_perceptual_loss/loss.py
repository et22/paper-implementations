...
class PerceptualLoss(nn.Module):
    ...
    def __init__(self):
        ...
        self.vgg16 = vgg16(weights = VGG16_Weights.IMAGENET1K_FEATURES)
    ...
    def forward(self, output, target):
        loss = self.style_weight * self.style_loss(output, target) \
                + self.content_weight * self.content_loss(output, target) \
                + self.tv_weight * self.tv_loss(output, target)
        return loss
    ...
    def style_loss(self, output, target):
        ...
        for out_feat, targ_feat in zip(output_features, target_features):
            ...
            loss += square(linalg.matrix_norm(g_out-g_tar))
